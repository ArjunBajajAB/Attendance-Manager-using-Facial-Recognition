...
#################################################################################################################
    ...
    def FirstPage(self):
        ...
        self.ExitButton = Button(self.content_frame, text="Exit", activebackground="grey", bd=3, bg="White",
                                  fg="Black",
                                  command= exit, font=self.ButtonFont, justify=RIGHT, height=1, width=8)
        self.ExitButton.place(x=600, y=240)
...
###################################################################################################################
    ...
    def AboutPage(self):
        ...
        self.about = "Attendance Manager \n\n This is an application that can mark as well as manage your attendance. It confirms your identity by opening your webcam and using our face recognition model to confirm your identity and automatically mark your attendance.Also you can check your previous attendance. \n This application is developed by Arjun Bajaj and Anirudh"
        self.AboutInfo = Message(self.content_frame, bg="Black", fg="White", font=self.TextFont, justify=CENTER,width=600, text=self.about)
        self.AboutInfo.place(x=0, y=0)


        self.BackButton = Button(self.content_frame, text="Back", activebackground="grey", bd=3, bg="White",fg="Black",command=self.FirstPage, font=self.ButtonFont, justify=CENTER, height=1, width=5)
...
######################################################################################################################
    ...
    def ProceedPage(self):
        ...
        self.EnterName = Entry(self.main_frame,bd=3,bg="black",fg="white")
        self.EnterName.place(x=790,y=170)

        self.EnrollmentNoLabel = Label(self.main_frame, text="Enrollment No :", bg="black", fg="white",font=self.TextFont)
        self.EnrollmentNoLabel.place(x=600, y=240)

        self.EnterEnrollmentNo = Entry(self.main_frame,bd=3,bg="black",fg="white")
        ...
        self.CourseEnter.config(bg="Black",fg="White")
        ...
        self.EnterSemester.config(bg="Black", fg="White")
        ...
        self.ResetButton = Button(self.main_frame, text="Reset", activebackground="grey", bd=3, bg="White",
                                         fg="Black", font=self.ButtonFont, justify=RIGHT, height=1, width=8,command=self.ProceedPage)
        self.ResetButton.place(x=780, y=420)

        self.ContinueButton= Button(self.main_frame, text="Continue", activebackground="grey", bd=3, bg="White",
                                         fg="Black", font=self.ButtonFont, justify=RIGHT, height=1, width=8,command=self.ContinuePage)
        self.ContinueButton.place(x=650, y=420)

        self.BackButton_Process = Button(self.main_frame, text="Back", activebackground="grey", bd=3, bg="White",fg="Black", font=self.ButtonFont, justify=RIGHT, height=1, width=8)
    ...
    def ContinuePage(self):
        ...
        if self.RollNo.isdigit() and int(len(self.RollNo)) == 11 and self.Name.isalpha() and self.Semester != "Select" and self.Course != "Select":
            if self.Course == "BCA" and self.Semester == "VII":
                ...
                self.create_MainFrame()
                self.Check_Your_AttendanceButton = Button(self.main_frame, text="Check your attendance", activebackground="grey", bd=3, bg="White",
                                         fg="Black", font=self.ButtonFont, justify=RIGHT, height=1, width=25,command=self.Check_AttendancePage)
                self.Check_Your_AttendanceButton.place(x=650, y=210)


                self.Mark_Your_AttendanceButton= Button(self.main_frame, text="Mark your attendance", activebackground="grey", bd=3, bg="White",
                                         fg="Black", font=self.ButtonFont, justify=RIGHT, height=1, width=25,command=self.Mark_AttendancePage)
            ...
            self.ProceedPage()
            self.Alert = Label(self.main_frame, text="!Your name should be name \n\n Enrollment number should be 11 digits only:", bg="black", fg="white", font=self.TextFont)
...
##############################################################################################################
    ...
    def Check_AttendancePage(self):
        ...
        if self.course == "BCA":
            if self.semester == "I":
                self.SubjectEnter = OptionMenu(self.main_frame, self.ClickedSubject, "Maths-I", "Techical Communication","C language","ICIT","Physic")
            elif self.semester == "II":
                self.SubjectEnter = OptionMenu(self.main_frame, self.ClickedSubject, "Maths-II", "POM","DE","DS","DBMS")
            elif self.semester == "III":
                self.SubjectEnter = OptionMenu(self.main_frame, self.ClickedSubject, "Maths-III", "CA", "FEDT", "POA", "C++")
            elif self.semester == "IV":
                self.SubjectEnter = OptionMenu(self.main_frame, self.ClickedSubject, "Maths-IV", "Web.Tech","Java","SE","CN")
            elif self.semester == "V":
                self.SubjectEnter = OptionMenu(self.main_frame, self.ClickedSubject, "E.com", "CG","PHP","OS","ST","BE")
            elif self.semester == "VI":
                self.SubjectEnter = OptionMenu(self.main_frame, self.ClickedSubject, "DWH & DM","MC","Linux","M&IA","BI","AI","NS","NP")
        elif self.course == "B.Tech":
            if self.semester == "I":
                self.SubjectEnter = OptionMenu(self.main_frame, self.ClickedSubject, "Maths-I", "Techical Communication","C language","ICIT","Physic-I")
            elif self.semester == "II":
                self.SubjectEnter = OptionMenu(self.main_frame, self.ClickedSubject, "Maths-II", "POM","DE","DS","DBMS")
            elif self.semester == "III":
                self.SubjectEnter = OptionMenu(self.main_frame, self.ClickedSubject, "Maths-III", "CA", "FEDT", "POA", "C++")
        ...
        self.DisplayAttendance= Button(self.main_frame, text="Display Attendance", activebackground="grey", bd=3, bg="White",
                                     fg="Black", font=self.ButtonFont, justify=RIGHT, height=1, width=16,command=self.ShowAttendance)
